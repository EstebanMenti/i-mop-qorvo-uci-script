...
- ✅ ~~Medición de distancia real con una segunda placa~~ — se probó vía `cli_bridge/`+`mixed_ranging.py` (ver nota abajo): conectividad y configuración de ambos lados funcionaron de punta a punta, pero **el ranging físico real todavía no se logró** (ambos lados reportan timeout). Sigue pendiente una medición exitosa, con una segunda placa UCI dedicada o resolviendo la interoperabilidad con la placa CLI.
...
**A diferencia de la conclusión original ("no sirve tal cual"), se construyó igual la herramienta** (`src/dwm3001c_uci/cli_bridge/` + `mixed_ranging.py`, ver [ranging-mixto-cli-uci.md](ranging-mixto-cli-uci.md)) aprovechando que CLI y UCI comparten el mismo motor FiRa por debajo (confirmado contra el Developer Manual del SDK) — es decir, no hace falta que `UciClient` le hable UCI a la placa CLI: alcanza con controlar cada placa con su propio protocolo (UCI local, CLI remota por BLE) y dejar que se encuentren por aire con parámetros FiRa alineados. **Resultado real (2026-09-03):** toda la plomería de control (BLE, encendido remoto, `RESPF`, `SESSION_SET_APP_CONFIG` local) funcionó sin errores, pero el ranging físico no se completó — ambos lados devuelven `RX_TIMEOUT`/`RANGING_RX_TIMEOUT` en cada ronda. Detalle completo, hipótesis para la próxima iteración, y por qué la distancia se mide del lado UCI (el puente BLE no reenvía notificaciones asíncronas no solicitadas) en [ranging-mixto-cli-uci.md](ranging-mixto-cli-uci.md).
