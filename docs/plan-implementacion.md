...
- ✅ `session_set_app_config()` — alcance ampliado (ver docs/protocolo-uci.md §2.2): 26 parámetros (`uci/app_config.py`, `AppConfigParam`), suficientes para levantar una sesión de TWR real **y** para medir distancia real contra otro dispositivo (ver F5 y `docs/ranging-mixto-cli-uci.md`), confirmado contra hardware. `get_app_config()`, `update_multicast_list()` y el resto de los ~64 parámetros restantes de `App.defs` quedan pendientes.
...
**Aceptación:** ✅ validado contra hardware real: `session_init()` → `Status.OK`, `get_session_state(handle)` → `Status.OK` con `SessionState.INIT`, `get_session_count()` refleja la sesión activa (1) y luego su ausencia (0) tras `session_deinit(handle)`. Se capturan las 2 notificaciones `SESSION_STATUS_NTF` esperadas (`INIT` y `DEINIT`), con `reason_code=0` (`STATE_CHANGE_WITH_SESSION_MANAGEMENT_COMMANDS`), coincidiendo con `SessionState`/`SessionStateChangeReason` de `uci/enums.py`. ✅ `session_set_app_config()` con los 26 parámetros devuelve `Status.OK` sin rechazos, y **desbloquea `RANGING_START` de verdad** (ver F5).
...
- ✅ `SESSION_SET_APP_CONFIG` (ver F4) implementado y confirmado como prerrequisito real y suficiente (con el conjunto de 26 parámetros) para que `RANGING_START` tenga éxito **y** para medir distancia real contra otro dispositivo.

**Aceptación:** ✅ validado contra hardware real de punta a punta, con sesión configurada: `ranging_start()` → `Status.OK`, el estado de sesión pasa a `SessionState.ACTIVE`, se ejecutan rondas de ranging reales (9 rondas en ~1 segundo con `ranging_interval_ms=200`, confirmado con `get_ranging_count()`) y se reciben `RANGING_DATA_NTF` reales en cada una (header decodificado correctamente: `session_handle` y `ranging_interval_ms` coinciden exactamente con lo configurado). `ranging_stop()` → `Status.OK`, estado vuelve a `SessionState.IDLE`. **Con solo los 5 parámetros "mandatory" (sin el resto del conjunto ampliado), `RANGING_START` seguía devolviendo `Status.ERROR_SESSION_NOT_CONFIGURED`** — hallazgo documentado en protocolo-uci.md §2.2. ✅ **Medición de distancia real validada** entre este `Controller`/`Initiator` (UCI) y un `Controlee`/`Responder` real (segunda placa DWM3001CDK con firmware CLI, controlada por BLE) — ver `docs/ranging-mixto-cli-uci.md`: rondas con `Status.OK` y distancia real en centímetros. `TwrMeasurement` (`core/models.py::parse_twr_measurement`) decodifica el prefijo de la primera medición dentro de `RANGING_DATA_NTF` (mac, status, NLOS, distancia); el resto de las mediciones cuando hay más de un `Controlee` sigue sin decodificarse.
...
**Aceptación:** ✅ validado contra hardware real: **11/11 checks OK**, repetido exitosamente tras un cambio de firmware UCI (2026-09-03). Reporte archivado en [docs/validaciones/2026-09-03-validacion-f0-f6.txt](validaciones/2026-09-03-validacion-f0-f6.txt) y resumido en [docs/resultados-validacion.md](resultados-validacion.md), siguiendo el patrón de `i-mop-qorvo-CLI-script`. `RANGING_START`/`RANGING_STOP` validan `Status.ERROR_SESSION_NOT_CONFIGURED` como resultado esperado (no `Status.OK`) porque `DEFAULT_SPECS` deliberadamente no llama a `SESSION_SET_APP_CONFIG` (fuera del alcance declarado de esta spec, ver `validation/spec.py`) — no porque el comando no exista: ya está implementado y confirmado desde F4/F5. Pendiente (no bloqueante): extender `DEFAULT_SPECS` para cubrir también `SESSION_SET_APP_CONFIG` y un `RANGING_START` que sí espere `Status.OK`.
...
- ✅ ~~Medición de distancia real con una segunda placa~~ — lograda vía `cli_bridge/`+`mixed_ranging.py` (ver nota abajo): ranging real confirmado contra hardware, con distancia medida en centímetros. Sigue pendiente (no bloqueante) una medición entre dos placas que corran **ambas** firmware UCI (sin CLI/BLE de por medio), para tener también ese caso cubierto.
...
### Nota: se investigó y se probó el puente BLE del proyecto hermano como segunda placa

Se investigó si la placa que anuncia por Bluetooth con nombre tipo `UWB-Node-N` (confirmado que existe al menos `UWB-Node-2`, visible en un escaneo BLE real) podía usarse como segundo dispositivo UWB para probar `RANGING_START`/`RANGING_STOP` con un par real. Tras revisar `i-mop-qorvo-CLI-script` (rama `hardware/ble-bridge-nrf52840`) y `I-mop-nrf52840-fw`, se confirmó que:

1. Ese setup es un DWM3001C cableado por UART a una placa **nRF52840 separada** que hace de puente BLE, con el firmware **CLI de texto** (`STAT`/`INITF`/...) — no el firmware UCI binario de este proyecto.
2. El protocolo sobre el enlace BLE es el shell de Zephyr sobre Nordic UART Service (comando `qorvo <texto>`, confirmado "IMPLEMENTADO Y VALIDADO EN HARDWARE REAL" en `I-mop-nrf52840-fw/doc/00_BLE_Protocol_Specification.md`) — no el framing binario `MT`/`PBF`/`GID`/`OID` de UCI.
3. Una vez montado el puente, el USB nativo de esa DWM3001C queda inaccesible para consola.

**A diferencia de la conclusión original ("no sirve tal cual"), se construyó igual la herramienta** (`src/dwm3001c_uci/cli_bridge/` + `mixed_ranging.py`, ver [ranging-mixto-cli-uci.md](ranging-mixto-cli-uci.md)) aprovechando que CLI y UCI comparten el mismo motor FiRa por debajo (confirmado contra el Developer Manual del SDK) — es decir, no hace falta que `UciClient` le hable UCI a la placa CLI: alcanza con controlar cada placa con su propio protocolo (UCI local, CLI remota por BLE) y dejar que se encuentren por aire con parámetros FiRa alineados. **Resultado real (2026-09-03):** tras seis corridas contra hardware real, se logró **ranging físico real con distancia medida** (rondas con `Status.OK`, distancia en centímetros). La causa de los cinco intentos fallidos anteriores (`RX_TIMEOUT`/`RANGING_RX_TIMEOUT` en el 100% de las rondas) no era un parámetro de `SESSION_SET_APP_CONFIG` sino que el `session_id` de la sesión UCI local nunca se pasaba también al `RESPF -ID` del lado CLI remoto — hallazgo obtenido comparando contra `uwb-qorvo-tools` (proyecto hermano independiente con esta misma arquitectura ya validada), no documentado en ninguna fuente oficial de Qorvo. Detalle completo de las seis corridas, la causa raíz, y por qué la distancia se mide del lado UCI (el puente BLE no reenvía notificaciones asíncronas no solicitadas) en [ranging-mixto-cli-uci.md](ranging-mixto-cli-uci.md).

**Conclusión práctica:** el ranging real entre dos dispositivos **no requiere** una segunda placa UCI dedicada — la placa CLI remota del banco BLE del proyecto hermano alcanza, siempre que `session_id` (UCI) y `RESPF -ID` (CLI) coincidan. Una segunda placa con firmware UCI seguiría siendo útil para aislar problemas futuros (descartar la mezcla CLI/UCI como variable), pero ya no es un bloqueante.
